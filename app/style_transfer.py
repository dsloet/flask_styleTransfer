...
from app.utils import load_img, deprocess_img
from app import app
...
vgg_weights = os.path.join("app", "static", "vgg19_weights_tf_dim_ordering_tf_kernels_notop.h5")


def download_vgg():
    ''' Downloads vgg19.h5 if it doesn't exist'''
    if not os.path.exists(os.path.join('app', 'static', 'vgg19_weights_tf_dim_ordering_tf_kernels_notop.h5')):
        print("vgg19.h5 doesn't exist")
        ...
        urllib.request.urlretrieve(
            ...
            os.path.join('app', 'static', 'vgg19_weights_tf_dim_ordering_tf_kernels_notop.h5'))
    else:
        print("vgg19.h5 is already there")
...
def get_content_loss(content_image, target_image):
  ...
  Compute the content loss for style transfer.
    Inputs:
    - base: features of the content image, Tensor with shape [height, width,
     channels]
...
def get_style_loss(base_style, gram_target):
    ...
    return tf.reduce_mean(tf.square(gram_base - gram_target)) / (n**2 * m)


def get_feature_representations(model, content_path,
                                style_path, num_content_layers):
    ...
    style_features = [style_layer[0] for style_layer
                      in style_outputs[num_content_layers:]]
...
def compute_loss(model, loss_weights, generated_output_activations,
                 gram_style_features, content_features, num_content_layers,
                 num_style_layers):

    generated_content_activations = \
        generated_output_activations[:num_content_layers]
    generated_style_activations = \
        generated_output_activations[num_content_layers:]
...
def get_model(content_layers,style_layers):
    ...
    # We don't need to (or want to) train any
    # layers of our pre-trained vgg model,
    # so we set it's trainable to false.
...
def run_style_transfer(content_path, style_path, num_iterations=200, content_weight=0.1, style_weight=0.9): 
    ...
    # Get the style and content feature 
    # representations (from our specified intermediate layers) 
    ...
    # In original paper, the initial stylized image
    # is random matrix of same size as that of content image
    # but in later images content image was used instead on
    # random values for first stylized image
    # because it proved to help to stylize faster
    generated_image = load_img(content_path)
    # generated_image = np.random.randint(0,255,
    # size=generated_image.shape)

    # Create tensorflow variable to hold a
    # stylized/generated image during the training
    ...
    loss = compute_loss(
        ...
        num_content_layers,
        num_style_layers)
    opt = tf.train.AdamOptimizer(learning_rate=9,
                                 beta1=0.9,
                                 epsilon=1e-1).minimize(loss[0], var_list = [generated_image])

    sess.run(tf.global_variables_initializer())
    sess.run(generated_image.initializer)

    # loading the weights again because
    # tf.global_variables_initializer() resets the weights
    vgg19.load_weights(vgg_weights)

    # Put loss as infinity before training starts and
    # Create a variable to hold best image (i.e image with minimum loss)
    ...
    for i in range(num_iterations):
        ...
        # Make sure image values stays in the
        # range of max-min value of VGG norm
        ...
        if total_loss < best_loss:
            ...
            # print best loss
            print('best: iteration: ', i, 'loss: ',
                  total_loss, '  style_loss: ', s_loss,
                  '  content_loss: ', c_loss)

            # Save image after every 100 iterations
            if (i+1) % 2 == 0:
                output = Image.fromarray(best_img)
                output.save(app.config['RESULT_FOLDER']+'/'+str(i+1)+'-'+save_name)
